...
class _EditDoctorScreenState extends State<EditDoctorScreen> {
  ...
  void initState() {
    ...
    _ratingController =
        TextEditingController(text: widget.doctor.rating.toString());
    _reviewCountController =
        TextEditingController(text: widget.doctor.reviewCount.toString());
    _healthCenterController =
        TextEditingController(text: widget.doctor.healthCenterId);
    _isVerified = widget.doctor.isVerified;
    selectedLanguages = widget.doctor.languages;
    availableServices = widget.doctor.selectedServices;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text('edit_doctor_details'.tr,
        ...
        child: Form(
          ...
          child: Column(
            children: [
              ...
              _buildTextField(
                  _specialtyController, 'specialty'.tr, Icons.medical_services),
              _buildTextField(
                  _experienceController, 'experience'.tr, Icons.timer,
                  isNumeric: true),
              _buildTextField(_biographyController, 'biography'.tr,
                  Icons.description_outlined,
                  maxLines: 3),
              _buildTextField(
                  _educationController, 'education'.tr, Icons.school),
              _buildTextField(_ratingController, 'rating'.tr, Icons.star,
                  isNumeric: true),
              _buildTextField(
                  _reviewCountController, 'review_count'.tr, Icons.reviews,
                  isNumeric: true),
              _buildTextField(_healthCenterController, 'health_center_id'.tr,
                  Icons.business),
              ...
              Wrap(
                spacing: 10,
                children: availableServices.map((language) {
                  return ChoiceChip(
                    label: Text(language),
                    selected: availableServices.contains(language),
                    selectedColor: Colors.green,
                    onSelected: (bool selected) {
                      setState(() {
                        if (selected) {
                          availableServices.add(language);
                        } else {
                          availableServices.remove(language);
                        }
                      });
                    },
                  );
...
//   List<String> selectedLanguages = [];
//
//   // Add services list and selected services
...
//     _isVerified = widget.doctor.isVerified;
//     selectedLanguages = widget.doctor.languages;
//     selectedServices = widget.doctor.selectedServices; // Initialize selected services
//     _imageUrl = widget.doctor.photoUrl;
//   }
//
//   Future<void> _pickImage() async {
...
//           'education': _educationController.text,
//           'languages': selectedLanguages,
//           'selectedServices': selectedServices, // Add selected services to update
//           'rating': double.parse(_ratingController.text),
//           'reviewCount': int.parse(_reviewCountController.text),
//           'healthCenterId': _healthCenterController.text,
//           'photoUrl': _imageUrl,
...
//               ),
//
//               // Services Selection (similar to languages)
//               SizedBox(height: 16),
//               Align(
//                 alignment: Alignment.centerLeft,
//                 child: Text('services'.tr, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
//               ),
//               Wrap(
//                 spacing: 10,
